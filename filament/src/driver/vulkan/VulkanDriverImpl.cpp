...
void acquireCommandBuffer(VulkanContext& context) {
    ...
    context.commands = &swap.commands;
...
void flushCommandBuffer(VulkanContext& context) {
    ...
    VkResult error = vkEndCommandBuffer(context.commands->cmdbuffer);
    ...
    VkSubmitInfo submitInfo {
        ...
        .pWaitDstStageMask = &waitDestStageMask,
        .commandBufferCount = 1,
        .pCommandBuffers = &context.commands->cmdbuffer,
    };
    ...
    error = vkResetCommandBuffer(context.commands->cmdbuffer, 0);
    ASSERT_POSTCONDITION(!error, "vkResetCommandBuffer error.");
    VkCommandBufferBeginInfo beginInfo {
        .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO,
        .flags = VK_COMMAND_BUFFER_USAGE_SIMULTANEOUS_USE_BIT,
    };
    error = vkBeginCommandBuffer(context.commands->cmdbuffer, &beginInfo);
